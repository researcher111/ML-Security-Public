...
- **≥4 distinct services compromised (30 pts)** — Metasploitable 3 (Linux) has well over 4. Common paths: ProFTPD 1.3.5 mod_copy (the walkthrough), UnrealIRCd 6697 backdoor, Apache/Drupal/web app, Samba, MySQL/CUPS, plus several sudo/SUID privescs. Penalize if all four are variations on one service, or if the student couldn't verify root with their own `id` output.
